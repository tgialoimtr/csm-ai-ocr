...
def getGender(rawstr):
    if rawstr in ['F', 'P']:
        return 'Female'
    elif rawstr in ['M', 'H', 'N']:
        return 'Male'
    elif rawstr in ['X']:
        return 'Unspecified'
    else:
...
def getNation(rawstr):
    if rawstr in _countries:
        return _countries[rawstr]
    else:
        return 'Unknown'
